...
class BarbatanaCaudal extends ObjectScene {
    constructor() {
        ...
        for (const s of shape) {
            switch (s.action) {
                case "line":
                    path.lineTo(s.values[0] - modify, s.values[1] - modify);
                    break;
                case "move":
                    path.moveTo(s.values[0] - modify, s.values[1] - modify);
                    break;
                case "curve":
                    path.quadraticCurveTo(s.values[0] - modify, s.values[1] - modify, s.values[2] - modify, s.values[3] - modify);
                    break;
                default: break;
            }
        }
